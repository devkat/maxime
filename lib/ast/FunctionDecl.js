...
FunctionDecl.prototype.populate = function(scope) {
  ...
  this.scope = scope.createChildScope(transcode.functionName(this.name.name));
  var that = this;
...
FunctionDecl.prototype.transcode = function() {
  return {
    ...
    id: { type: 'Identifier', name: transcode.functionName(this.name.name) },
...
FunctionDecl.prototype.transcodeAsMember = function() {
  return {
    key: { type: 'Identifier', name: transcode.functionName(this.name.name) },
    value: {
  ...
  return {
    ...
    expression: {
      ...
      left: {
        ...
        property: { type: 'Identifier', name: transcode.functionName(this.name.name) }
      },
      right: {
        type: 'FunctionExpression',
        params: this.params.map(function(p) { return p.transcode(); }),
