...
ClassDecl.prototype.populate = function(scope) {
  ...
  var classScope = scope.createChildScope('_' + this.name.name, true);
  
  var typeParams = this.typeParams.map(function(p) {
    return p.populate(classScope);
  });
  
  var features = this.features.map(function(f) {
    return f.populate(classScope);
  });
  ...
  var clazz = new Class(classScope, className, typeParams, features, ctors, methods, this.loc);
